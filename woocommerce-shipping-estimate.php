...
class Plugin {
	...
	public function render_estimate_label( $label, $method ) {
		...
		$method_estimate_weekday   = get_option( 'wc_shipping_method_estimate_weekday', [] );
		...
		$weekday_setting = isset ($method_estimate_weekday[$method_id]) && $method_estimate_weekday[$method_id];
		...
		if ( 'dates' === get_option( 'wc_shipping_estimate_format', 'days' ) ) {
			$label = $this->generate_delivery_estimate_dates( $days_from_setting, $days_to_setting, $label, $method ,$weekday_setting);
		} else {
			$label = $this->generate_delivery_estimate_days( $days_from_setting, $days_to_setting, $label, $method ,$weekday_setting);
		}
	 ...
	 */
	private function generate_delivery_estimate_days( $days_from_setting, $days_to_setting, $label, $method , $weekdays_setting) {
		...
		$estimate_label = $this->get_estimate_label( $days_to, $weekdays_setting );
		...
		if ( ! empty( $days_from_setting ) && ! empty( $days_to_setting ) ) {
			...
			$estimate_label = $this->get_estimate_label( $days_from, $weekdays_setting );
	 ...
	 */
	private function generate_delivery_estimate_dates( $days_from_setting, $days_to_setting, $label, $method ,$weekday_setting) {
		...
		$days_to   = apply_filters( 'wc_shipping_estimate_dates_to', date_i18n( 'F j', strtotime( $days_to_setting . $weekday_setting?'weekdays':'days' ) ) );
	 ...
	 */
	private function get_estimate_label( $days, $weekdays_setting ) {

		$estimate_label = _n( $weekdays_setting?'weekday':'day', $weekdays_setting?'weekdays':'days', $days, 'woocommerce-shipping-estimate' );
				...
				<?php foreach ( $zones as $zone_id => $zone_data ) : ?>
					...
					<thead>
						<tr style="background: #e9e9e9;">
							<th colspan="5" style="text-align: center; border: 1px solid #e1e1e1;">
						...
						<tr>
							...
							<th class="weekday"><?php esc_html_e( 'Weekdays?', 'woocommerce-shipping-estimate' ); ?> <?php echo wc_help_tip( __( 'Count only weekdays.', 'woocommerce-shipping-estimate' ) ); ?></th>
					...
					<?php foreach ( $zone->get_shipping_methods() as $instance_id => $method ) : ?>
						<tr>
							...
							<td class="weekday">
								<input type="checkbox" name="method_estimate_weekday[<?php echo esc_attr( $instance_id ); ?>]" value="1" <?php checked(1, isset($method_estimate_weekday[$instance_id]) && $method_estimate_weekday[$instance_id], true);?> />
							</td>
						</tr>
					<?php endforeach; ?>
					</tbody>
	...
	public function process_method_estimate() {
		...
		$estimate_weekday = isset( $_POST['method_estimate_weekday'] )?$_POST['method_estimate_weekday']:'' ;

		$method_estimate_from = [];
		$method_estimate_to   = [];
		$method_estimate_weekday = [];
		...
		if ( is_array( $estimate_weekday ) && count( $estimate_weekday ) > 0 ) {

			foreach ( $estimate_weekday as $method_id => $value ) {
				if ( $value ) {
					$value = true;
				}

				$method_estimate_weekday[ $method_id ] = $value;
			}
		}
		...
		update_option( 'wc_shipping_method_estimate_weekday', $method_estimate_weekday );
	}
